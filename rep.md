...
Simulator-wide services are scoped to the simulator, not to any individual asset, and must not be bundled into asset files. The following interfaces are explicitly prohibited at the asset level:

*   `/clock` topic (`rosgraph_msgs/msg/Clock`): simulation-time publication is a single, simulator-wide service. Each simulator must expose exactly one `/clock` publisher, configured by the simulator runtime (not by asset authors). Assets must not declare `RosTopicAPI` for `/clock`, and must not carry execution-layer equivalents (e.g., OmniGraph `ROS 2 Publish Clock`, Gazebo `libgazebo_ros_clock`) in any layer — including proprietary layers such as `isaac.usda` or `gazebo.usda`. A simulator encountering such an artifact must ignore it and emit a distinct warning, deferring to its runtime `/clock` service.
*   Any interfaces included in the `simulation_interfaces` package (e.g., entity spawning, simulation lifecycle, world state, state reset). These describe the simulator itself and must be provided by the simulator runtime, never by the assets it loads.

The prohibition covers both declarative `Ros*API` authoring and any execution-layer equivalent in proprietary layers. Simulators must provide `/clock` and `simulation_interfaces` as first-class runtime services configured outside any loaded asset (e.g., via launch files or simulator configuration), so that a single `/clock` is published regardless of how many assets are loaded into the scene.
