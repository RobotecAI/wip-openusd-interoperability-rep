...
*   **Composition Over Invention:** Features that overlap with existing neutral schemas must compose with them rather than duplicate their attributes. For example, a tool-mount site must compose with `IsaacSiteAPI` (Section 1.3.5) for the named-frame primitive and rely on inclusion in `IsaacRobotAPI.robotLinks` (Section 2.7) for TF broadcast membership; it must not invent its own axis-convention or broadcast-flag attributes. Named poses must reference `UsdPhysicsJoint` prims by relationship and defer naming to the Robot Schema name-override attributes (Section 2.10) rather than carrying duplicate strings. Gripper schemas must declare their attached bodies via `UsdPhysicsJoint` / `UsdCollectionAPI` and interface with ROS via `RosTopicAPI`/`RosServiceAPI`/`RosActionAPI` rather than inventing a parallel transport.
...
*   Mount-point / site schemas. `IsaacSiteAPI` is already cited in Section 1.3.5 as the canonical named-auxiliary-frame primitive for the Robot Schema and is referenced from Section 2.7 as the TF-broadcast primitive for non-physical frames, so it is not a registry submission. Future site-style extensions (e.g., specialised calibration-reference schemas) must compose with `IsaacSiteAPI` rather than invent a parallel axis-convention or broadcast-flag attribute. TF membership is declared by inclusion in `IsaacRobotAPI.robotLinks` per Section 2.7; a registry schema must not duplicate that membership mechanism.
